...
- `.github/coding-conventions.md` — 規約の SSOT（判断基準として使用）
- `GEMINI.md` — Gemini へのガードレール（sandbox の扱いなど）
...
`.github/coding-conventions.md` と `GEMINI.md` を参照し、各指摘について以下を判断する。
...
- `apps/sandbox/` への指摘は限定的に評価する（プロトタイプのため）
- `apps/web/src/components/ui/` 自体が Radix UI を import するのは正常
- 日本語コミット・絵文字禁止は意図的な規約であり指摘不要
- セキュリティ指摘（XSS・シークレット漏洩）は sandbox でも `valid`
