...
修正が必要な指摘がある場合、`/commit` スキルの手順に従ってコードを修正・コミットする（**ここではまだ push しない**）。

> **重要 — push とスレッド resolve の順序**:
> `Unresolved Review Check` は **push（synchronize）時点**のスレッド状態で判定される。
> push した後に返信・resolve すると、チェックは未解決状態のまま fail し、ジョブの手動再実行が必要になる。
> 修正コミットがある場合は必ず以下の順序で進めること：
>
> 1. コードを修正してコミットする（返信に記載するコミットハッシュはこの時点で確定する）
> 2. ステップ 4（返信）およびステップ 5（resolve）を完了させる
> 3. ステップ 5 の完了後に `git push` を実行する（resolve 済みの状態で CI が実行される）
>
> すでに push 済みで後から resolve した場合は、`gh run rerun {run_id} --failed` で
> Unresolved Review Check を再実行すれば回復できる。
