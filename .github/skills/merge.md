...
修正が必要な指摘がある場合、`/commit` スキルの手順に従ってコードを修正・コミットする。
...
>
> 1. コードを修正してコミットする（**push はまだしない**。返信に記載するコミットハッシュはこの時点で確定する）
> 2. 全コメントへ返信する（ステップ 4）
> 3. 全スレッドを resolve する（ステップ 5）
> 4. 最後に push する（resolve 済みの状態で CI が実行される）
>
