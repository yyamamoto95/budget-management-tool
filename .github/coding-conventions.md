...
テストファイルは実装コードと同等の品質基準でレビューする。以下の観点を重点的に確認すること。

| 観点 | チェック内容 |
|------|------------|
| **命名** | テスト名（`describe` / `it` / `test`）が「何を・どういう条件で・どうなるか」を明確に表しているか |
| **デッドコード** | 実行されない `expect`・到達しないテストケース・コメントアウトされたテストが残っていないか |
| **テスト品質** | 実装の変更を検知できる意味のある検証をしているか（実装詳細ではなく振る舞いをテストしているか） |
| **カバレッジの妥当性** | 正常系だけでなく異常系・境界値がカバーされているか |
| **ファイル構成** | テスト対象のモジュール構成と対応したディレクトリ・ファイル名になっているか |
| **共通規約の適用** | `any` 禁止・未使用変数削除・命名規約など、通常のコード規約を同様に適用する |
