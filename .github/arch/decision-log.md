...
## ADR-008: ドキュメント体系の統廃合とドキュメント統治の導入
...
- 常設ドキュメントを `.github/sprint-protocol.md` セクション 6「SSOT マップ」の closed list に限定する
- 旧構想ドキュメント（`core-spec.md` / `product-challenges.md`: 黒背景・断罪 UI の「Xデー」構想）を削除。現行プロダクト方針の SSOT は `product-identity.md` に一本化する
- スナップショット型ドキュメント（`arch/issues-and-improvements.md`、`arch/aws-deployment-architecture.md`、`design/review_alignment_with_core_value.md`）を削除。一過性の分析・提案は GitHub Issue に書く
- `ai-agent-workflow.md` / `multi-agent-roles.md` / `project-overview.md` を `sprint-protocol.md` ほか既存 SSOT に統合、`commit-message-instructions.md` + `pull-request-instructions.md` を `git-conventions.md` に統合、`operation/` 3 ファイルを `operation/README.md` に統合、`architecture/ddd_rules.md`（Express/TypeORM 前提で陳腐化）を削除（DDD 規約は `coding-conventions.md` が SSOT）
- CI に Docs Guard（`pr-checks.yml`）を追加し、新規 `.md` の追加を `docs-approved` ラベルなしではブロックする

### 背景

`.github/` 配下だけで 6,000 行超のドキュメントが存在し、相互矛盾（旧構想 vs 現行方針、migrate dev の可否、TypeORM/Prisma 表記揺れ）と重複（テスト規約が 4 箇所、SSOT マップが 3 箇所、スキル定義の完全コピー）が発生していた。AI エージェントが矛盾した規約を読み込むリスクと保守コストが増大していた。

### 影響

- インフラ構成の SSOT は `infra/` の Terraform コードと本 decision-log。構成図ドキュメントは維持しない
- 新しい常設ドキュメントの追加にはユーザー承認（`docs-approved` ラベル）と SSOT マップの同時更新が必要
