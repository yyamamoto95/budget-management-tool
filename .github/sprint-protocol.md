...
| フィールド | GitHub Variables | 既定 ID | 更新タイミング |
|-----------|------------------|--------|--------------|
| `Sprint #` | `PROJECT_SPRINT_FIELD_ID` | `PVTF_lAHOA-qlQM4BWtuozhSdS10` | PR マージ時（GH Actions）/ スプリントクロージング時（AI） |
| `Story Points` | `PROJECT_SP_FIELD_ID` | `PVTF_lAHOA-qlQM4BWtuozhSdS2Q` | PR マージ時（GH Actions）/ バックフィル時（AI） |
| `Cycle Time (h)` | `PROJECT_CYCLE_TIME_FIELD_ID` | `PVTF_lAHOA-qlQM4BWtuozhSdS2U` | PR マージ時（GH Actions）/ バックフィル時（AI） |
| `Status` | `PROJECT_STATUS_FIELD_ID` | `PVTSSF_lAHOA-qlQM4BWtuozhR_pd4` | ラベル変更時（GH Actions `project-board-sync.yml`） |

Project ID は GitHub Variables `PROJECT_ID` を優先し、未設定時は既定 ID `PVT_kwHOA-qlQM4BWtuo` を使用する。
Status 選択肢 ID は `PROJECT_STATUS_BACKLOG` / `PROJECT_STATUS_SPRINT_BACKLOG` / `PROJECT_STATUS_IN_REVIEW` / `PROJECT_STATUS_DONE` を優先する。
...
| 6 | `.github/multi-agent-roles.md` | マルチエージェント役割・任意のコード理解ツール活用ルール |
| 7 | `.github/PRD/product_requirements.md` | プロダクト要件・優先度 |
| 7a | `.github/PRD/user-stories.md` | BDD ユーザーストーリー・プロトタイプ検証チェックリスト |
| 7b | `.github/PRD/investment-business-basics.md` | 家計管理と投資支援を接続する新規事業の立ち上げ論点 |
| 7c | `.github/product-identity.md` | ミッション・提供価値・プロダクトの軸 |
| 7d | `.github/product-challenges.md` | 課題分析（core-spec の前提） |
| 7e | `.github/core-spec.md` | コアロジック＆UI仕様（家計の寿命） |
| 8 | `.github/design/` | UI/UX・設計ドキュメント |
| 9 | `.github/project-overview.md` | プロジェクト概要（ディレクトリ構成・ユビキタス言語・技術スタック） |
| 10 | `.github/arch/` | 技術決定の ADR（decision-log.md）・DDD 規範（ddd_rules.md）・AWS 構成 |
| 11 | `.github/operation/` | 運用手順（DB 更新フロー・認証セキュリティ・ユーザー運用） |
...
- **一時点のレビュー結果・作業記録を `.md` として保存しない**。経緯は Issue / PR に、技術決定は `.github/arch/decision-log.md`（ADR）に記録する。恒久ドキュメントには「現在の決定と意図」のみを書く
...
6. **スプリントクロージング**:
   ...
   - **velocity-log.json 同期失敗時の対応**: `update-velocity.yml` は PR 本文内の複数 `Closes #N` を処理する。同期に失敗した場合のみ、原因を確認して手動補記する
...
2. **各 AI ツールには薄いラッパーを配置する**
   ...
   - Codex → `.codex/skills/{skill-name}/SKILL.md`（frontmatter + `.github/skills/X.md` への参照）
