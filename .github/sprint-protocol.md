...
| 7b | `.github/spec/core-spec.md` | コアロジック＆UI仕様（生活余力・統計比較・最速入力） |
| 7c | `.github/spec/investment-business-basics.md` | 家計管理と投資支援を接続する新規事業の立ち上げ論点 |
...
| 11 | `.github/rules/operation/` | 運用手順（DB 更新フロー・認証セキュリティ・ユーザー運用・AWS 構成） |
| — | `.github/retrospective-template.md` | Retro Issue 本文テンプレート＋クロージング手順（/sprint-close が参照） |
| — | `apps/api/prisma/schema.prisma` | DB スキーマのデータ SSOT（`docs/database/schema.dbml` は自動生成） |
| — | `packages/api-spec/openapi.yaml` | API スキーマのデータ SSOT（Route 定義から自動生成） |
| — | `.github/design/figma-tokens.json` | デザイントークンのデータ SSOT（`pnpm sync:tokens` で globals.css へ反映） |
