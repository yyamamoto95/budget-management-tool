...
```jsonc
{ "$schema": "https://unpkg.com/@yyamamoto95/flow-wireframe/schema/flow-wireframe.schema.json", ... }
```
...
（`flow-wireframe-v{version}` タグの push で GitHub Actions がテスト・ビルド・`npm publish` を実行する）。
